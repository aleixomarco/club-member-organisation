      ...
      {offen && (
        <div className="mt-1 rounded-lg overflow-hidden" style={{ border: `1px solid ${C.line}`, background: C.glass }}>
          <input autoFocus value={suche} onChange={(e) => setSuche(e.target.value)}
            placeholder="Namen suchen …" aria-label={t("feld.nutzerSuchen")}
      ...
      <div className="px-4 pt-3">
        {!canPost ? (
          ...
          <div>
            ...
            <button onClick={send} aria-label="Nachricht senden" className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: C.red }}><Send size={16} color={C.aufPrimaer} /></button>
            </div>
          </div>
        )}
      </div>
