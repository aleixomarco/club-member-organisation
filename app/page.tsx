          ...
          <div className="p-4">
            ...
            {laeuft && (
              <div className="rounded-2xl p-3.5 mt-3" style={{ background: C.fehlerFlaeche, border: `1px solid ${C.edge}` }}>
                ...
                {anzeige.aktion_url && <a href={anzeige.aktion_url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between rounded-xl px-3 py-2.5 text-xs font-bold mt-2.5" style={{ background: C.red, color: C.aufPrimaer }}>Zur Aktion <ExternalLink size={14}/></a>}
              </div>
            )}
...
function TrainerTeamSettings({ user, members, setMembers }) {
  ...
  return <div className="rounded-2xl p-4 mb-5" style={{ background: C.glass, border: `1px solid ${C.line}` }}>
    ...
    {loading ? <div className="text-xs py-3" style={{ color: C.textDim }}>{t("tm.laden")}</div> : <>
      ...
      <div className="pt-3 mb-3" style={{ borderTop: `1px solid ${C.line}` }}><div className="text-xs font-bold" style={{ color: C.ink }}>{t("tm.kapitaenZuweisen")}</div><div className="text-[10px]" style={{ color: C.textDim }}>Die Kapitänsrolle gilt immer für die ausgewählte Mannschaft.</div></div>
