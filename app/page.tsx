...
const C = {
  ...
  /* Lesbare Schrift- und Symbolfarbe AUF den Vereinsfarben. Immer diese
     benutzen statt fest "#fff" - sonst verschwindet der Text, sobald ein
     Verein eine helle Farbe waehlt. */
  aufPrimaer: "var(--club-primary-kontrast)",
  aufSekundaer: "var(--club-secondary-kontrast)",
  /* Zweitfarbe als Schrift auf HELLER Flaeche - Gegenstueck zu aufDunkel. */
  sekundaerAufHell: "var(--club-secondary-readable)",
...
/* Erfolg oder Fehler - woran erkennt man das?
 *
 * Frueher entschied das der Text selbst: message.includes("gespeichert")
 * faerbte den Hinweis gruen, alles andere rot. Auf Deutsch ging das gut.
 * Sobald dieselbe Meldung aber "Salvato" oder "Kaydedildi" heisst, findet
 * die Suche nichts mehr - und JEDE Erfolgsmeldung waere in sechs Sprachen
 * rot. Ein geglueckter Speichervorgang saehe aus wie ein Fehlschlag.
 *
 * Deshalb traegt eine Erfolgsmeldung jetzt ein unsichtbares Zeichen am
 * Anfang (U+200B, Breite null). Es haengt an der Meldung, nicht an ihrer
 * Sprache, und wird beim Anzeigen wieder entfernt. */
const OK_ZEICHEN = "\u200B";
...
const NOTIFICATION_OPTIONS = [
  /* Diese sechs muessen Wort fuer Wort so heissen, wie notify_event_audience
     sie verschickt - der Schluessel IST die Art der Meldung. Genau daran ist
     es vorher gescheitert: Die Einstellung hiess training_created, verschickt
     wurde 'training', und deshalb hat kein einziger Schalter je gewirkt. */
  ["training_created", "Neues Training"], ["training_cancelled", "Trainingsabsage"],
  ["training_changed", "Trainingsänderung"],
...
/* t kommt als Parameter, nicht aus dem Haken.
   ...
   die Uebersetzungen eingesetzt hat, hat den Unterschied nicht gesehen und
   den Haken hier hineingeschrieben; der Linter hat es gemeldet. */
function registrierFehlerText(error, t) {
...
/* t kommt als Parameter, nicht aus dem Haken.
   ...
   die Uebersetzungen eingesetzt hat, hat den Unterschied nicht gesehen und
   den Haken hier hineingeschrieben; der Linter hat es gemeldet. */
function zeileZuNachricht(zeile, t) {
  const profil = Array.isArray(zeile.profiles) ? zeile.profiles[0] : zeile.profiles;
  const name = profil?.full_name || t("rol.mitgliedLabel");
...
/* ------------------------------------------------------------------ */
/* Die Schriftfarbe richtet sich nach der Flaeche.
   Vorher stand hier fest C.white. Auf einer hellen Vereinsfarbe - Gelb,
   Hellblau, oder der Zweitfarbe Weiss, die in vier Vorgaben vorkommt - war die
   Beschriftung damit unsichtbar. Weil die Farben als CSS-Variablen kommen,
   laesst sich die Helligkeit hier nicht ausrechnen; verglichen wird deshalb,
   WELCHE Farbe uebergeben wurde. */
function Pill({ children, bg, fg, style }) {
  const lesbar = bg === C.red ? C.aufPrimaer : bg === C.secondary ? C.aufSekundaer : C.white;
  return (
    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold tracking-wide"
      style={{ background: bg, color: fg || lesbar, fontFamily: "Inter", ...style }}>
      {children}
    </span>
  );
}
...
function FeatureRow({ icon: Icon, title, subtitle, onClick, accent, locked }) {
  ...
  return (
    <button onClick={onClick} className="w-full flex items-center justify-between px-4 py-3.5 rounded-2xl mb-2.5" style={{ background: C.glass, border: `1px solid ${C.edge}`, boxShadow: "0 10px 26px rgba(60,30,45,0.07)" }}>
      ...
      {locked
        ? <Lock size={15} aria-label={t("sys.nichtFreigeschaltet")} style={{ color: C.textDim, flexShrink: 0 }} />
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col items-center justify-center text-center px-7">
        ...
        <div className="text-xs mb-5 leading-snug" style={{ color: C.textDim }}>{t("allg.gesperrt")}</div>
        <button onClick={goSubscribe} className="px-5 py-2.5 rounded-2xl text-xs font-bold" style={{ background: C.red, color: C.aufPrimaer, boxShadow: `0 8px 20px color-mix(in srgb, ${C.red} 34%, transparent)` }}>{t("allg.mehr")}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col h-full px-6 pt-10 pb-6 overflow-y-auto" style={{ background: C.paper }}>
      ...
      <button onClick={onWeitererVerein} className="w-full py-3 rounded-xl text-sm font-bold mb-3" style={{ background: C.ink, color: C.white, fontFamily: "Inter" }}>
        {mitgliedschaften.length === 0 ? t("verein.suchenOderAnlegen") : t("verein.weiteremBeitreten")}
      </button>
  ...
  return (
    <div className="flex flex-col h-full px-6 pt-10 pb-6 overflow-y-auto" style={{ background: C.paper }}>
      <div className="flex flex-col items-center mb-8">
        ...
        <div className="text-xl text-center" style={{ fontFamily: "Oswald", fontWeight: 700, color: C.ink }}>{t("verein.finden")}</div>
        <div className="text-xs text-center mt-1" style={{ color: C.textDim, fontFamily: "Inter" }}>{t("verein.findenHinweis")}</div>
      </div>
      ...
      <div className="space-y-2 mb-6">
        ...
        {geladen === null ? (
          <div className="text-xs" style={{ color: C.textDim, fontFamily: "Inter" }}>{t("verein.laden")}</div>
        ) : geladen === false ? (
      ...
      {/* Kein t("allg.zurueck") mehr. Wer hier steht, ist angemeldet und hat noch keinen
          ...
          Der einzige echte Ausweg ist t("allg.abmelden"), und der steht oben. */}
    </div>
  );
}
  ...
  return (
    <div className="mb-3">
      ...
      <div className="flex items-center gap-2 rounded-xl px-3 py-2.5" style={{ background: isCustom ? C.fehlerFlaeche : C.paperDim, border: isCustom ? `1px solid ${primary}` : "1px solid transparent" }}>
        <span className="text-xs font-bold flex-1" style={{ color: C.ink }}>{t("verein.eigeneFarben")}</span>
        <input aria-label={t("verein.primaerfarbe")} type="color" value={primary} onChange={(e) => onChange(e.target.value, secondary)} className="w-8 h-8 rounded-lg cursor-pointer" style={{ border: "none", padding: 0, background: "none" }} />
        <input aria-label={t("verein.sekundaerfarbe")} type="color" value={secondary} onChange={(e) => onChange(primary, e.target.value)} className="w-8 h-8 rounded-lg cursor-pointer" style={{ border: "none", padding: 0, background: "none" }} />
      </div>
...
function NewClubScreen({ onCreate, goBack }) {
  ...
  return (
    <AuthShell footer={<div className="text-center text-xs" style={{ color: C.textDim, fontFamily: "Inter" }}><button onClick={goBack} className="font-bold" style={{ color: C.red }}>{t("verein.zurueckAuswahl")}</button></div>}>
      <div className="text-xl mb-1" style={{ fontFamily: "Oswald", fontWeight: 600, color: C.ink }}>{t("verein.neuAnlegen")}</div>
      <div className="text-xs mb-5" style={{ color: C.textDim, fontFamily: "Inter" }}>{t("verein.erstesKonto")}</div>
      <form onSubmit={submit}>
        ...
        <button type="submit" disabled={busy} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm" style={{ background: C.red, color: C.aufPrimaer, fontFamily: "Inter", fontWeight: 700, opacity: busy ? .65 : 1 }}>
          {busy ? t("allg.wirdAngelegt") : <>{t("verein.anlegen")} <ArrowRight size={15} /></>}
        </button>
      </form>
    </AuthShell>
  );
...
function KontoLoeschenBlock({ onDelete }) {
    ...
    try {
      ...
      setBusy(false);
        ...
        /* Zurueckhaltend, aber auffindbar. Apple verlangt einen Weg zur
            ...
            sein: Ein roter Kasten neben t("allg.abmelden") laesst zwei harmlose
  ...
  return (
    <>
      {!confirming ? (
        <button onClick={() => setConfirming(true)} className="w-full py-2 text-[11px] underline" style={{ color: C.textDim, fontFamily: "Inter" }}>{t("pf.kontoLoeschen")}</button>
      ) : (
      ...
      <div className="mx-auto w-full" style={{ maxWidth: 380 }}>
        ...
        <div className="rounded-3xl p-5 mb-4" style={{ background: C.glass, border: `1px solid ${C.edge}`, boxShadow: "0 18px 40px rgba(60,30,45,0.08)" }}>
          <div className="text-lg font-bold mb-2" style={{ fontFamily: "Oswald", color: C.ink }}>
            {waiting ? t("mit.wartenAufFreigabe") : t("mit.keineMitgliedschaft2")}
          </div>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-5">
        ...
        {/* t("verein.wechseln") gibt es nur, wenn ueberhaupt einer gewaehlt ist.
            Seit die Anmeldung vorn steht, ist das der Ausnahmefall - man kommt
            nur noch ueber die Vereinssuche hierher. */}
        {club && <button onClick={goChangeClub} className="text-[11px] font-bold px-2.5 py-1 rounded-full" style={{ background: C.paperDim, color: C.textDim, fontFamily: "Inter" }}>{t("verein.wechseln")}</button>}
      </div>
      ...
      {eigeneMitgliedschaft && (
        <div className="mb-5">
          ...
          >
            <div className="w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-bold flex-shrink-0" style={{ background: C.red, color: C.aufPrimaer, fontFamily: "Inter" }}>{initialsOf(eigeneMitgliedschaft.display_name || offeneSitzung?.email || "?")}</div>
            <div className="text-left flex-1 min-w-0">
              ...
              <div className="text-[11px] truncate" style={{ color: C.textDim, fontFamily: "Inter" }}>
                {uebernahmeLaeuft ? t("allg.wirdGeoeffnet") : eigeneMitgliedschaft.status === "pending" ? t("mit.aufnahmeOffen") : t("login.alsKontoFortfahren")}
              </div>
            </div>
      ...
      {!supabase && members.filter((m) => !m.accountPending).length > 0 && <div className="mt-8">
        <div className="text-xs uppercase tracking-widest font-semibold mb-2.5" style={{ color: C.textDim, fontFamily: "Inter" }}>{t("login.demo")}</div>
...
function RegisterScreen({ onRegister, members, club, goLogin }) {
  ...
  /* Ohne vorgewaehlten Verein wird nur das KONTO angelegt. Der Verein kommt
     ...
     Vorher fuehrte t("login.registrieren") ohne Verein in die Vereinssuche. Das
  ...
  const submit = async (e) => {
    ...
    const fehlt = [
      [!form.firstName.trim(), t("reg.vorname")],
  ...
  return (
    <AuthShell
      club={club}
      footer={<div className="text-center text-xs" style={{ color: C.textDim, fontFamily: "Inter" }}>{t("reg.schonMitglied")}{" "}<button onClick={goLogin} className="font-bold" style={{ color: C.red }}>{t("reg.jetztAnmelden")}</button></div>}
    >
      ...
      <form onSubmit={submit}>
        {/* Getrennte Felder statt eines einzigen t("feld.vollerName").
        ...
        <Field icon={User} placeholder={t("reg.vorname")} value={form.firstName} onChange={set("firstName")} />
        ...
        <Field icon={Lock} type="password" placeholder={t("ph.passwortBestaetigen")} value={form.password2} onChange={set("password2")} />

        <label className="flex items-start gap-2 mb-3"><input type="checkbox" checked={legalAccepted} onChange={(e) => setLegalAccepted(e.target.checked)} className="mt-0.5"/><span className="text-[11px]" style={{ color: C.textDim, fontFamily: "Inter" }}>{t("reg.akzeptiere1")}<a href="/nutzungsbedingungen" target="_blank" rel="noreferrer" style={{ color: C.red, fontWeight: 700 }}>{t("recht.nutzung")}</a>{t("reg.akzeptiere2")}<a href="/datenschutz" target="_blank" rel="noreferrer" style={{ color: C.red, fontWeight: 700 }}>{t("recht.datenschutzerklaerung")}</a>{t("reg.akzeptiere3")}</span></label>
  ...
  if (!nextEvent) {
    return (
      <div className="rounded-3xl p-5 mb-6 relative overflow-hidden" style={{ background: `linear-gradient(160deg, color-mix(in srgb, ${C.red} 82%, #fff) 0%, ${C.red} 100%)` }}>
        ...
        <div className="relative text-white text-sm" style={{ fontFamily: "Inter" }}>{t("home.keinSpiel")}</div>
      </div>
    );
  }
...
function Dashboard({ user, members, events, channels, news, dutyPlan, seasonVotes, tippPredictions, tippResults, polls, setPolls, onVote, onUnvote, umfrageFokus, onUmfrageFokusErledigt, onFavoritMannschaft, werbeplaetze, onSponsorImpression, onSponsorClick, goEvents, goSeason, goTipp, goDuty, goNews, goTasks, goVehicles, currentClub, featureEnabled, dashboardTileOrder, entitlement, goSubscribe, mannschaften = [], gewaehlteMannschaft = "", onMannschaftWechsel }) {
  ...
  let dutySubtitle = isFormalMember(user) ? t("ph.stationenBeispiel") : t("allg.nurMitglieder");
  if (isFormalMember(user)) {
    ...
    dutySubtitle = assigned && zugehoeriger ? `${assigned.station} · ${formatDate(zugehoeriger.date)}` : assigned ? assigned.station : t("allg.jetztEintragen");
  }
  ...
  return (
    <div className="px-4 pt-4 pb-24">
      <div className="flex items-start justify-between mb-4">
        <div>
          <div className="mb-1" style={{ fontFamily: "Inter", color: C.textDim, fontSize: 13 }}>{t("home.willkommenKomma")}</div>
      ...
      {geburtstageHeute.length > 0 && (
        <div className="flex items-center gap-2 rounded-xl px-3 py-2.5 mb-5" style={{ background: C.sekundaerWeich, border: `1px solid ${C.edge}` }}>
          ...
          <div className="text-sm" style={{ fontFamily: "Inter", color: C.ink }}><b>{t("home.geburtstag")}</b> {geburtstageHeute.join(" · ")} 🎉</div>
        </div>
      )}
      ...
      {offen && darfListeSehen && (
        <div className="rounded-xl overflow-hidden mb-2" style={{ border: `1px solid ${C.line}` }}>
          ...
          {!laedt && (liste || []).length === 0 && (
            <div className="px-3 py-2.5 text-[11px]" style={{ color: C.textDim }}>{t("helf.niemandEingeteilt")}</div>
          )}
          {!laedt && (liste || []).map((z, i) => (
            <div key={z.membership_id} className="flex items-center justify-between gap-2 px-3 py-2"
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full flex-shrink-0"
                ...
                {z.status === "zugesagt" ? t("ev.zugesagtLabel") : t("ev.abgesagtLabel")}
              </span>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-lg p-2 mt-1" style={{ background: C.paperDim }}>
          ...
          <div className="flex gap-1.5 items-start flex-wrap">
            ...
            <select value={eintragStation} onChange={(e) => setEintragStation(e.target.value)}
              ...
              <option value="">{t("ph.station")}</option>
...
function CarpoolSection({ ev, currentUser }) {
  ...
  const removeCarpool = async (carpoolId) => {
    if (!window.confirm(t("ev.fahrgemeinschaftEntfernenFrage"))) return;
    setMessage("");
  ...
  return (
    <div className="mt-2 mb-1">
      ...
      {message && <div className="text-[11px] mt-1.5" style={{ color: C.red }}>{meldungstext(message)}</div>}
    </div>
  );
}
        ...
        <div className="px-4 pb-4">
          ...
          {ev.home !== true && (
            ...
            <button onClick={() => onCarpool(ev.id)} className="w-full flex items-center justify-center gap-2 py-2 rounded-lg text-xs mb-1"
              ...
              <Car size={14} /> {carpoolOn ? t("fzg.platzAngeboten") : t("ev.platzAnbieten")}
            </button>
          ))}
...
function EventsView({ onNeuLaden, currentUser, members, events, setEvents, carpools, setCarpools, dutyPlan, setDutyPlan, onDienstSetzen, werbeplaetze, onSponsorImpression, onSponsorClick, focusRequest, onFocusApplied, currentClub, featureEnabled, entitlement, goSubscribe }) {
  ...
  /* Der Filter ist immer sichtbar. Frueher erschien er erst, nachdem man auf
     t("ev.spiele2") oder t("ev.training") umgeschaltet hatte - in der Voreinstellung "alle",
  ...
  return (
    <div className="px-4 pt-4 pb-24">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <button onClick={() => setCalendarOpen((v) => !v)} aria-pressed={calendarOpen} aria-label={t("aria.kalenderUmschalten")} className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: calendarOpen ? C.ink : C.paperDim, color: calendarOpen ? C.white : C.textDim }}><CalendarDays size={15}/></button>
    ...
    if (supabase && isDbId(active.id) && isDbId(user.authProfileId)) {
      ...
      if (gespeichert) {
        setChannels((cs) => cs.map((c) => c.id !== active.id || c.messages.some((m) => m.id === gespeichert.id)
          ? c
          : { ...c, messages: [...c.messages, { ...zeileZuNachricht(gespeichert, t), me: true }].slice(-200) }));
      }
    } else {
    ...
    <div className="pt-4 flex flex-col" style={{ height: "100%", paddingBottom: "calc(var(--erg-nav-h, 96px) + 18px)" }}>
      ...
      {blocked.length > 0 && (
        <div className="mx-4 mb-3 rounded-xl px-3 py-2" style={{ background: C.paperDim }}>
          <div className="text-[10px] font-bold mb-1.5" style={{ color: C.textDim }}>{t("chat.ausgeblendet")}</div>
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-3">
        {visibleMessages.map((m, i) => {
          /* Nach Kennung vergleichen, nicht nach Namen: Datenbank-Nachrichten
             ...
             eigene Nachrichten als fremde - samt t("chat.melden") und t("chat.blockieren") an
          ...
          /* Den Namen aus der Mitgliederliste holen, nicht aus der Nachricht.
             ...
             zeileZuNachricht fiel auf t("rol.mitgliedLabel") zurueck - in jedem Verein, bei
    ...
    <div className="px-4 pt-4 pb-24">
      ...
      {showForm && (
        <div className="rounded-2xl p-3 mb-5 space-y-2.5" style={{ background: C.glass, border: `1px solid ${C.line}` }}>
          <div className="text-xs font-bold" style={{ fontFamily: "Inter", color: C.ink }}>{editingPost ? t("news.bearbeiten") : t("news.neu")}</div>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t("ph.titel")}
...
/* Die Abfrage nach der Aufnahme in den Verein.
 *
 * WARUM UEBERHAUPT FRAGEN
 * Ohne sie merkt niemand, dass es die Einstellung gibt. Die eigene Mannschaft
 * laeuft ohnehin von allein - der Punkt ist die andere Richtung: dass man von
 * der U15 hoeren kann, in der das eigene Kind spielt, ohne dort Mitglied zu
 * sein.
 *
 * WORAN SIE ERKENNT, DASS SIE SCHON GESTELLT WURDE
 * Nicht an einem Merker, sondern an den Zeilen selbst: "Uebernehmen" schreibt
 * fuer JEDE Mannschaft eine Zeile, auch fuer die abgewaehlten. Danach ist die
 * Frage beantwortet und kommt nicht wieder. Das ist der Grund, warum auch die
 * unveraenderten Vorgaben geschrieben werden - eine Zeile "alles wie
 * voreingestellt" sieht sinnlos aus, ist aber genau der Unterschied zwischen
 * "noch nicht gefragt" und "gefragt und so gewollt".
 *
 * "Spaeter" merkt sich der Browser. Das reicht: Wer die Frage wegschiebt,
 * soll sie beim naechsten Mal wiedersehen, aber nicht in derselben Sitzung. */
function TeamMeldungenAbfrage({ currentUser, clubId }) {
  const t = useT();
  const [mannschaften, setMannschaften] = useState(null);
  const [wahl, setWahl] = useState({});
  const [speichert, setSpeichert] = useState(false);
  const [fehler, setFehler] = useState("");
  const [weg, setWeg] = useState(false);
  const merker = `cmo.teamMeldungen.spaeter.${currentUser?.id}`;

  useEffect(() => {
    if (!supabase || !isDbId(currentUser?.id) || !isDbId(clubId)) return;
    try { if (window.localStorage.getItem(merker)) { setWeg(true); return; } } catch { /* Privatmodus */ }
    let abgebrochen = false;
    (async () => {
      const [{ data: vorhanden }, { data: teams }, { data: meine }] = await Promise.all([
        supabase.from("team_benachrichtigungen").select("team_id").eq("membership_id", currentUser.id).limit(1),
        supabase.from("teams").select("id,name,category").eq("club_id", clubId).eq("active", true).order("name"),
        supabase.from("team_members").select("team_id").eq("membership_id", currentUser.id),
      ]);
      if (abgebrochen) return;
      if ((vorhanden || []).length || !(teams || []).length) { setWeg(true); return; }
      const eigene = new Set((meine || []).map((z) => z.team_id));
      setMannschaften((teams || []).map((team) => ({ ...team, eigen: eigene.has(team.id) })));
      setWahl(Object.fromEntries((teams || []).map((team) => [team.id, {
        aktiv: eigene.has(team.id), spiele: true, trainings: true, ergebnisse: true,
      }])));
    })();
    return () => { abgebrochen = true; };
  }, [currentUser?.id, clubId, merker]);
  ...
  const uebernehmen = async () => {
    ...
    const { error } = await supabase.from("team_benachrichtigungen").upsert(
      mannschaften.map((team) => ({
        membership_id: currentUser.id, team_id: team.id,
        aktiv: wahl[team.id].aktiv, spiele: wahl[team.id].spiele,
        trainings: wahl[team.id].trainings, ergebnisse: wahl[team.id].ergebnisse,
      })), { onConflict: "membership_id,team_id" });
    setSpeichert(false);
    if (error) { setFehler(t("tmb.fehler")); return; }
    setWeg(true);
  };

  if (weg || !mannschaften) return null;

  const stellen = (teamId, feld, wert) =>
    setWahl((alt) => ({ ...alt, [teamId]: { ...alt[teamId], [feld]: wert } }));
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center p-3" style={{ background: "rgba(20,21,26,.72)" }}>
      <div className="w-full max-w-md rounded-3xl p-4 max-h-[86vh] overflow-y-auto"
        ...
        <div className="space-y-2.5">
          {mannschaften.map((team) => (
            <div key={team.id} className="rounded-2xl p-3" style={{ background: C.glass, border: `1px solid ${C.line}` }}>
              <button type="button" onClick={() => stellen(team.id, "aktiv", !wahl[team.id].aktiv)}
                className="w-full flex items-center justify-between gap-3">
                <span className="text-left">
                  <span className="block text-xs font-bold" style={{ color: C.ink }}>{team.name}</span>
                  <span className="block text-[10px]" style={{ color: C.textDim }}>
                    {team.eigen ? t("tmb.meineMannschaft") : (team.category || "")}
                  </span>
                </span>
                <span className="w-10 h-6 rounded-full flex items-center px-0.5 shrink-0"
                  style={{ background: wahl[team.id].aktiv ? C.erfolg : C.line, justifyContent: wahl[team.id].aktiv ? "flex-end" : "flex-start" }}>
                  <span className="w-5 h-5 rounded-full" style={{ background: C.white }}/>
                </span>
              </button>
              {wahl[team.id].aktiv && (
        ...
        {fehler && <div role="status" className="text-[11px] mt-3" style={{ color: C.fehler }}>{fehler}</div>}
        <div className="flex gap-2 mt-4">
          <button type="button" onClick={spaeter} className="px-4 py-2.5 rounded-xl text-xs font-bold" style={{ background: C.glass, color: C.textDim }}>{t("tmb.abfrageSpaeter")}</button>
          <button type="button" onClick={uebernehmen} disabled={speichert} className="flex-1 py-2.5 rounded-xl text-xs font-bold" style={{ background: C.ink, color: C.white }}>
            {speichert ? t("allg.wirdBearbeitet") : t("tmb.abfrageSpeichern")}
          </button>
        </div>
      </div>
    </div>
...
/* Wer die Mannschaft fuehrt.
 *
 * Im Kader standen bisher nur die Athletinnen und Athleten - wer die
 * Mannschaft trainiert, betreut oder als Kapitaen fuehrt, war nirgends zu
 * sehen. Genau danach sucht aber, wer eine Frage zur Mannschaft hat.
 *
 * Die Zuordnung steht in team_members.function und kommt ueber die
 * Mitgliederliste mit. Die Spalte laesst genau vier Werte zu: spieler,
 * trainer, kapitaen, teammanager. Ein "Team-Staff" gibt es im System nicht -
 * dafuer braeuchte es einen fuenften Wert samt Regel, Rollenvergabe und
 * Rechten; das ist eine eigene Aenderung, keine Anzeige. */
function MannschaftsFunktionen({ team, members }) {
  const t = useT();
  const zeilen = [
    ["trainer", memberTrainerTeams],
    ["teammanager", memberManagedTeams],
    ["kapitaen", memberCaptainTeams],
  ].map(([rolle, teamsVon]) => ({
    rolle,
    namen: (members || [])
      .filter((mitglied) => teamsVon(mitglied).includes(team.name))
      .map((mitglied) => mitglied.name)
      .filter(Boolean)
      .sort((a, b) => a.localeCompare(b, "de")),
  }));

  return (
    <div className="rounded-2xl p-3.5 mb-4" style={{ background: C.paperDim, border: `1px solid ${C.line}` }}>
      <div className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: C.red }}>
        {t("tm.funktionen")}
      </div>
      <div className="space-y-1.5">
        {zeilen.map(({ rolle, namen }) => (
          <div key={rolle} className="flex items-baseline gap-2">
            <span className="text-[11px] font-bold flex-shrink-0" style={{ color: C.textDim, minWidth: 96 }}>
              {rollenLabel(t, rolle)}
            </span>
            <span className="text-xs" style={{ color: namen.length ? C.ink : C.textDim, fontStyle: namen.length ? "normal" : "italic" }}>
              {namen.length ? namen.join(", ") : t("tm.nichtBesetzt")}
            </span>
          </div>
        ))}
      </div>
...
function MeldungsSchalter({ label, wert, an, onChange }) {
  return (
    <button type="button" onClick={() => onChange(!wert)} disabled={!an}
      className="w-full flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl"
      style={{ background: C.glass, opacity: an ? 1 : 0.45 }}>
      <span className="text-xs font-bold" style={{ color: C.ink }}>{label}</span>
      <span className="w-9 h-5 rounded-full flex items-center px-0.5"
        style={{ background: wert && an ? C.erfolg : C.line, justifyContent: wert && an ? "flex-end" : "flex-start" }}>
        <span className="w-4 h-4 rounded-full" style={{ background: C.white }}/>
      </span>
    </button>
  );
}

/* Benachrichtigungen zu EINER Mannschaft.
 *
 * Der Voreinstellung liegt keine gespeicherte Zeile zugrunde, sondern eine
 * Regel: eigene Mannschaft ja, fremde nein. Die Datenbank rechnet genauso
 * (team_meldung_erlaubt), und beide muessen dasselbe sagen - sonst zeigt die
 * App etwas anderes an, als tatsaechlich verschickt wird. Deshalb steht die
 * Regel hier noch einmal und nicht als geratener Wert.
 *
 * Gespeichert wird sofort bei jeder Aenderung. Ein Speichern-Knopf waere hier
 * eine Falle: Vier Schalter sehen nicht nach einem Formular aus, und wer
 * weiterblaettert, haette seine Auswahl verloren. */
function MannschaftsMeldungen({ teamId, currentUser, imTeam }) {
  const t = useT();
  const [zeile, setZeile] = useState(null);
  const [laedt, setLaedt] = useState(true);
  const [fehler, setFehler] = useState("");
  const aktivDb = !!supabase && isDbId(currentUser.id) && isDbId(teamId);

  useEffect(() => {
    if (!aktivDb) { setLaedt(false); return; }
    let abgebrochen = false;
    (async () => {
      setLaedt(true);
      const { data } = await supabase.from("team_benachrichtigungen")
        .select("aktiv,spiele,trainings,ergebnisse")
        .eq("membership_id", currentUser.id).eq("team_id", teamId).maybeSingle();
      if (abgebrochen) return;
      setZeile(data || { aktiv: !!imTeam, spiele: true, trainings: true, ergebnisse: true });
      setLaedt(false);
    })();
    return () => { abgebrochen = true; };
  }, [aktivDb, teamId, currentUser.id, imTeam]);

  const setzen = async (feld, wert) => {
    const naechste = { ...zeile, [feld]: wert };
    setZeile(naechste);
    setFehler("");
    const { error } = await supabase.from("team_benachrichtigungen").upsert({
      membership_id: currentUser.id, team_id: teamId,
      aktiv: naechste.aktiv, spiele: naechste.spiele,
      trainings: naechste.trainings, ergebnisse: naechste.ergebnisse,
      geaendert_am: new Date().toISOString(),
    }, { onConflict: "membership_id,team_id" });
    if (error) { setFehler(t("tmb.fehler")); setZeile(zeile); }
  };

  if (!aktivDb) return null;
  if (laedt) return <div className="text-[11px] py-2" style={{ color: C.textDim }}>{t("tmb.laden")}</div>;

  return (
    <div className="rounded-2xl p-3.5 mb-4" style={{ background: C.paperDim, border: `1px solid ${C.line}` }}>
      <div className="text-xs font-bold mb-0.5" style={{ color: C.ink }}>{t("tmb.titel")}</div>
      <div className="text-[10px] mb-2.5" style={{ color: C.textDim }}>{t("tmb.hinweis")}</div>
      <div className="space-y-1.5">
...
function TeamsView({ currentUser, members, setMembers, currentClub }) {
  ...
  const removePlayerPenalty = async (penalty) => {
    if (!window.confirm(t("straf.entfernenFrage"))) return;
    setPenaltyMessage("");
    ...
    setPenaltyMessage(t("straf.wurdeEntfernt"));
  };
  const togglePlayerPenaltyPaid = async (penalty) => {
    ...
    setPenaltyMessage(nextPaid ? t("straf.alsBezahlt") : t("straf.alsOffen"));
  };
  ...
  const openPlayer = (player) => {
    ...
    setPlayerMessage("");
    /* Beide Abschnitte offen. Solange die Athletenansicht ein kleines Blatt am
       unteren Rand war, mussten sie zu sein - sonst haette das Blatt den
       halben Bildschirm eingenommen, bevor man ueberhaupt etwas wollte. Jetzt
       ist es eine ganze Seite, und zugeklappt landet man auf einer leeren
       Flaeche mit zwei Pfeilen, die man erst finden muss. Wer eine Athletin
       oeffnet, will ihre Mannschaften sehen - nicht erst danach fragen. */
    setTeamsOpen(true);
    setPenaltyOpen(true);
  };
  ...
  const createTeam = async (event) => {
    ...
    if (databaseMembership) {
      ...
      setTeams((current) => [...current, { id: createdId, name: name.trim(), category: category.trim() || t("tm.mannschaft"), is_adult: isAdultTeam }].sort((a, b) => a.name.localeCompare(b.name, "de")));
    }
    setName(""); setCategory(""); setIsAdultTeam(false); setShowCreate(false); setSelectedTeamId(createdId); setMessage((OK_ZEICHEN + t("tm.wurdeAngelegt"))); setSaving(false);
  };
  ...
  const archiveTeam = async () => {
    ...
    setSelectedTeamId(""); setEditingTeam(false); setMessage((OK_ZEICHEN + t("tm.wurdeArchiviert"))); setArchivingTeam(false);
  };
  const TeamCard = ({ team }) => {
...
function TeamPenaltyCatalog({ user }) {
  ...
  const addRule = async (event) => {
    ...
    setTitle(""); setAmount(""); setEditingId(""); setMessage((OK_ZEICHEN + t("straf.regelGespeichert"))); setSaving(false);
  };
  const editRule = (rule) => {
  ...
  const toggleAssignmentPaid = async (assignment) => {
    ...
    setMessage(nextPaid ? (OK_ZEICHEN + t("straf.alsBezahlt")) : (OK_ZEICHEN + t("straf.alsOffen")));
  };
  ...
  return <div className="rounded-2xl p-4 mb-5" style={{ background: C.glass, border: `1px solid ${C.line}` }}>
    ...
    {loading ? <div className="text-xs py-3" style={{ color: C.textDim }}>{t("tm.laden")}</div> : teams.length === 0 ? <div className="text-xs rounded-xl p-3" style={{ background: C.paperDim, color: C.textDim }}>Der Strafenkatalog ist nur für Erwachsenenmannschaften verfügbar. Dir ist aktuell keine Erwachsenenmannschaft als Athlet/in, Kapitän/in, Trainer/in oder Teammanager/in zugeordnet.</div> : <>
      ...
      {canManageSelectedTeam && (
        <div className="pt-4 mt-4" style={{ borderTop: `1px solid ${C.line}` }}>
          ...
          {databaseMembership && <button type="button" onClick={() => setShowHistory((v) => !v)} className="text-[11px] font-bold mb-2" style={{ color: C.ink }}>{showHistory ? t("sais.historieAusblenden") : t("sais.historieAnzeigen")}</button>}
          {showHistory && (
    ...
    {message && <div role="status" className="text-[11px] mt-2" style={{ color: istErfolg(message) || istErfolg(message) || istErfolg(message) || istErfolg(message) || istErfolg(message) ? C.erfolg : C.fehler }}>{meldungstext(message)}</div>}
  </div>;
}
...
function TasksView({ currentUser, members }) {
  ...
  const createTask = async (teamId) => {
    ...
    resetForm(); setShowCreateClub(false); setShowCreateTeamId(""); setMessage((OK_ZEICHEN + t("auf.wurdeAngelegt")));
    await loadAll();
  };
...
function VehiclesView({ currentUser, currentClub }) {
  ...
  const cancelBooking = async (booking) => {
    if (!window.confirm(t("fzg.stornoFrage"))) return;
    ...
    <div className="px-4 pt-4 pb-24">
      ...
      {showAddVehicle && (
        <div className="rounded-2xl p-4 mb-4" style={{ background: C.glass, border: `1px solid ${C.line}` }}>
          <div className="text-sm font-bold mb-2">{editingVehicleId ? t("fzg.bearbeiten") : t("fzg.neues")}</div>
          <input value={newVehicle.label} onChange={(e) => setNewVehicle({ ...newVehicle, label: e.target.value })} placeholder={t("ph.fahrzeugName")} className="w-full px-3 py-2.5 rounded-xl text-xs outline-none mb-2" style={{ background: C.paperDim }}/>
        ...
        {bookings.map((b) => (
          <div key={b.id} className="flex items-center gap-3 rounded-xl px-3 py-2.5"
            ...
            {b.status === "angefragt" && (
              darfEntscheiden ? (
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <button onClick={() => entscheide(b, true)} aria-label={t("aria.anfrageAnnehmen")}
                    className="w-8 h-8 rounded-lg flex items-center justify-center"
                    style={{ background: C.erfolgFlaeche, border: `1px solid ${C.erfolgRand}` }}>
                    <Check size={15} style={{ color: C.erfolg }} />
                  </button>
                  <button onClick={() => entscheide(b, false)} aria-label={t("aria.anfrageAblehnen")}
                    className="w-8 h-8 rounded-lg flex items-center justify-center"
        ...
        <div className="fixed inset-0 z-50 flex items-end p-3" style={{ background: "rgba(20,21,26,.72)" }} onClick={() => { setSelectedVehicle(null); setEditingBookingId(null); }}>
          <div role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()} className="w-full rounded-3xl p-5 max-h-[85%] overflow-y-auto" style={{ background: C.glass }}>
            ...
            {bookingForm.isPrivate ? (
              <input value={bookingForm.privateLabel} onChange={(e) => setBookingForm({ ...bookingForm, privateLabel: e.target.value })} placeholder={t("ph.nameZweck")} className="w-full px-3 py-2.5 rounded-xl text-xs outline-none mb-3" style={{ background: C.paperDim }}/>
            ) : (
                ...
                <div className="flex flex-col gap-1.5">
                  {viewingPhones.filter(Boolean).map((phone, i) => (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      <button onClick={() => copyPhone(phone)} aria-label={t("aria.telefonKopieren")} className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: C.glass, color: copiedPhone === phone ? C.secondary : C.textDim }}>
...
function DutyTemplatesPanel({ currentUser, sport }) {
  ...
  const createTemplate = async () => {
    ...
    setNewName(""); setMessage(t("help.satzAngelegt")); setMessageOk(true); await loadTemplates();
  };
        ...
        {anfrage ? (
          ...
          formularOffen ? (
            <form onSubmit={absenden} className="rounded-2xl p-4 mb-5 space-y-2.5" style={{ background: C.glass, border: `1px solid ${C.line}` }}>
              ...
              <input type="number" min="1" value={form.expected_accounts} onChange={(e) => setForm({ ...form, expected_accounts: e.target.value })} placeholder={t("ph.wieVieleZugaenge")} className="w-full px-3 py-2.5 rounded-xl text-xs outline-none" style={{ background: C.paperDim }} />
              ...
              <div className="flex gap-2">
                <button type="submit" disabled={sendet} className="flex-1 py-2.5 rounded-xl text-xs font-bold" style={{ background: C.ink, color: C.white, opacity: sendet ? .6 : 1 }}>{sendet ? t("allg.wirdGesendet2") : t("zug.anfrageSenden")}</button>
...
function SysAdminUserManager({ members, setMembers }) {
  ...
  const saveProfile = async () => {
    ...
    setMessage((OK_ZEICHEN + t("verein.profilGespeichert"))); setSaving(false);
  };
  const togglePlayerTeam = (teamId) => {
    setMessage("");
  ...
  const savePlayerTeams = async () => {
    ...
    setMembers((items) => items.map((member) => member.id === selected.id ? { ...member, playerTeams: names, team: hoechsteMannschaft(names) || t("rol.mitgliedLabel"), teams: [...new Set([...names, ...(member.trainerTeams || []), member.managedTeam].filter(Boolean))] } : member));
    setSavedPlayerTeamIds(playerTeamIds); setMessage((OK_ZEICHEN + t("tm.athletenMannschaftenGespeichert"))); setSaving(false);
  };
  ...
  return <div>
    ...
    {/* Suchfeld statt Auswahlliste.
        ...
        wird ueber Name UND Rollen, damit t("rol.vorstand") weiter zum Ziel fuehrt,
    ...
    {/* Zurueck zur Nutzeruebersicht.
        Wer einen Nutzer geoeffnet hatte, kam nur ueber das Suchfeld zurueck -
        man musste erst tippen, um die Liste wiederzusehen. Ein Weg zurueck,
        der aussieht wie einer, fehlte ganz.
        In Vereinsfarbe, weil er die Hauptbewegung auf diesem Bildschirm ist. */}
    {selected && (
...
function ProfileDataSettings({ user, setMembers, saveRef }) {
  ...
  const [form, setForm] = useState({
    ...
    /* Die Anmeldeadresse steht NICHT mehr in dieser Liste.
     *
     * Vorher wurde sie hineingeschrieben, wenn noch keine Kontaktadresse
     * hinterlegt war - bearbeitbar wie jede andere. Das vermischte zwei
     * verschiedene Dinge: die Adresse, mit der man sich anmeldet (sie liegt
     * in der Anmeldeverwaltung und wurde von diesem Formular ohnehin nie
     * geschrieben), und zusaetzliche Kontaktadressen in profiles.
     * Wer die erste Zeile aenderte, glaubte seine Anmeldeadresse zu aendern -
     * tatsaechlich legte er nur eine Kontaktadresse an und konnte sich
     * weiterhin nur mit der alten anmelden. */
    emails: user.contactEmails || [], phones: user.contactPhones?.length ? user.contactPhones : [user.phone || ""],
  ...
  const save = async () => {
    ...
    /* Die Rueckfrage vor dem Speichern ist entfallen. Sie warnte davor, sich
       aus den Vereinsnachrichten auszusperren - aber die Anmeldeadresse steht
       hier gar nicht mehr zur Bearbeitung, und eine zusaetzliche
       Kontaktadresse zu aendern sperrt niemanden aus. */
...
function NotificationSettings({ user, setMembers, saveRef }) {
  ...
  return <div>{message&&<div role="status" className="mb-4 text-[11px] rounded-xl px-3 py-2" style={{background:istErfolg(message)?C.erfolgFlaeche:C.fehlerFlaeche,color:istErfolg(message)?C.erfolg:C.fehler}}>{meldungstext(message)}</div>}{/* Die Push-Karte erscheint nur ausserhalb der nativen App. In der
    ...
    {/* Bis zum 04.09.2026 stand hier "&& !Capacitor.isNativePlatform()" - der
        Schalter erschien also ueberall AUSSER in der App aus dem App Store.
        Genau dort sitzen aber die echten Nutzer, und nur dort kann sich ein
...
function CalendarSyncSettings({ user, saveRef }) {
  ...
  const sync = async () => {
    ...
    const row = data?.[0];
    setToken(row?.token || "");
    setMessage((OK_ZEICHEN + t("kal.verbindungAktualisiert")));
  };
    ...
    <div className="rounded-2xl p-4" style={{ background: C.glass, border: `1px solid ${C.line}` }}>
      ...
      {alleTeams.length > 0 && <>
        ...
        <div className="space-y-1.5 mb-3">
          ...
          {teams.length > 0 && <button onClick={() => { setTeams([]); setMessage(""); }} className="w-full py-2 rounded-xl text-[11px] font-bold" style={{ background: C.glass, color: C.textDim }}>{t("kal.auswahlAufheben")}</button>}
        </div>
      </>}

      <div className="text-[10px] font-bold uppercase tracking-widest mb-1.5" style={{ color: C.textDim }}>{t("kal.aktualisierung")}</div>
            ...
            <div key={z.team_id} className="flex items-center justify-between gap-2 px-3 py-2.5"
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px]" style={{ color: C.textDim }}>
                  {z.aktiv ? `${z.teilnehmer} ${z.teilnehmer === 1 ? t("prot.teilnehmerKurz") : t("ev.teilnehmende")}` : t("tipp.keines")}
                </div>
              </div>
              <button onClick={() => umschalten(z)}
                ...
                {z.aktiv ? t("tipp.freigegeben") : t("mit.freigeben")}
              </button>
            </div>
          ))}
        </div>
              ...
              return (
                <div key={t.id} className="flex items-center gap-2 px-2.5 py-2 rounded-lg" style={{ background: C.paper }}>
                  <button onClick={() => onToggleTask(protocol.id, t.id)} className="w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: t.done ? C.secondary : "transparent", border: `1.5px solid ${t.done ? C.secondary : C.line}` }}>
                    {t.done && <Check size={10} color={C.aufSekundaer} />}
                  </button>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="rounded-2xl p-3 space-y-2.5" style={{ background: C.glass, border: `1px solid ${C.line}` }}>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t("ph.sitzungTitel")}
            className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ background: C.paperDim, fontFamily: "Inter", color: C.ink }} />
          ...
          <div key={slot.key} className="rounded-2xl p-3" style={{ background: C.glass, border: `1px solid ${C.line}` }}>
            ...
            <div className="text-[11px] mb-2.5" style={{ color: C.textDim }}>
              {!on ? t("sp.platzAusgeblendet")
                : sichtbar ? <>{t("sp.zuSehen")} <span style={{ color: C.ink, fontWeight: 600 }}>{sichtbar.titel}</span></>
                : t("allg.platzLeer")}
            </div>
            ...
            {!imBearbeiten && (
              <button onClick={() => bearbeiten(slot.key)} className="w-full px-3 py-2.5 rounded-xl text-xs" style={{ background: meiner ? C.paperDim : C.ink, color: meiner ? C.ink : C.white, fontWeight: 700, border: `1px solid ${C.line}` }}>
                {meiner ? t("sp.bearbeiten") : t("sp.eigenenEintragen")}
              </button>
            )}
            ...
            {imBearbeiten && (
              <div className="space-y-2 pt-1">
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button onClick={speichern} disabled={speichert} className="flex-1 px-3 py-2.5 rounded-xl text-xs" style={{ background: C.ink, color: C.white, fontWeight: 700, opacity: speichert ? .6 : 1 }}>{speichert ? "…" : t("allg.speichern")}</button>
                </div>
...
function PollManagerPanel({ polls, setPolls, clubId, onAnlegen, onUmschalten }) {
  ...
  const create = async () => {
    ...
    /* Die Benachrichtigung kommt jetzt aus der Datenbank (Ausloeser
       ...
       Bliebe er stehen, bekaeme jetzt jeder zwei Meldungen fuer dieselbe
...
function RolesPanel({ members, setMembers }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {message&&<div className="rounded-xl px-3 py-2 text-[11px] font-semibold" style={{background:C.fehlerFlaeche,color:C.fehler}}>{meldungstext(message)}</div>}
      {members.map((m) => {
...
function ClubLogoPanel({ club, onLogoUpdated }) {
  ...
  const uploadLogo = async (event) => {
    ...
    if (!supabase) {
      const reader = new FileReader();
      reader.onload = () => { onLogoUpdated(String(reader.result)); setMessage((OK_ZEICHEN + t("verein.logoGespeichert"))); setBusy(false); };
    ...
    onLogoUpdated(logoUrl); setMessage((OK_ZEICHEN + t("verein.logoGespeichert"))); setBusy(false);
  };
...
function ClubColorPanel({ club, onColorsUpdated }) {
  ...
  const save = async () => {
    setSaving(true); setMessage("");
    if (!supabase) { onColorsUpdated(primary, secondary); setMessage((OK_ZEICHEN + t("verein.farbenGespeichert"))); setSaving(false); return; }
    ...
    setMessage((OK_ZEICHEN + t("verein.farbenGespeichert")));
  };
  ...
  return <div className="rounded-2xl p-4 mt-3" style={{ background: C.glass, border: `1px solid ${C.line}` }}>
    ...
    <button onClick={save} disabled={saving || !dirty} className="w-full py-2.5 rounded-xl text-xs font-bold" style={{ background: dirty ? C.ink : C.paperDim, color: dirty ? C.white : C.textDim, opacity: saving ? .6 : 1 }}>{saving ? t("allg.wirdGespeichert") : t("verein.farbenSpeichern")}</button>
    {message && <div className="text-[11px] mt-2" role="status" style={{ color: istErfolg(message) ? C.erfolg : C.fehler }}>{meldungstext(message)}</div>}
  </div>;
}
...
function MembershipApprovalsPanel({ club, members, setMembers }) {
  ...
  /* Sperren und Entsperren.
     ...
     ueber t("allg.entfernen") geloescht, faellt die Sperre mit ihr weg - dann darf die
  ...
  return <div>
    <div className="rounded-2xl p-4 mb-4" style={{ background: C.primaerWeich, border: `1px solid ${C.edge}` }}>
      <div className="flex items-center justify-between gap-3">
        <div><div className="text-sm font-bold" style={{ color: C.ink }}>{t("mit.offeneAntraege")}</div><div className="text-[11px] mt-1" style={{ color: C.textDim }}>{t("mit.sofortAnmelden")}</div></div>
    ...
    {message && <div role="status" className="text-xs rounded-xl px-3 py-2.5 mb-3" style={{ background: istErfolg(message) ? C.erfolgFlaeche : C.fehlerFlaeche, color: istErfolg(message) ? C.erfolg : C.fehler }}>{meldungstext(message)}</div>}
...
function ClaimManagedPlayerPanel({ members, setMembers, currentUser }) {
  ...
  const merge = async () => {
    ...
    setManagedId(""); setRealId(""); setMessage((OK_ZEICHEN + t("mit.profileZusammengefuehrt"))); setSaving(false);
  };
  ...
  return (
    <div className="px-4 pt-4 pb-24">
      ...
      {/* Zwei Spalten statt waagerecht scrollend.
          ...
          t("verein.profil") suchte, musste erst wischen, um zu sehen, dass es das
...
export default function ClubMemberOrganisationApp() {
  ...
  /* Neu laden, sobald die App wieder im Vordergrund ist.
     ...
     Klick auf t("verein.wechseln") und zurueck half, aber darauf kommt niemand.
  ...
  useEffect(() => {
    ...
    const laden = async () => {
      ...
      for (const zeile of (nachrichten || []).reverse()) {
        (proKanal[zeile.channel_id] ||= []).push(zeileZuNachricht(zeile, t));
      }
    ...
    const kanal = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (ereignis) => {
        ...
        setChannels((cs) => cs.map((c) => c.id !== neu.channel_id || c.messages.some((m) => m.id === neu.id)
          ? c
          : { ...c, messages: [...c.messages, zeileZuNachricht(neu, t)].slice(-200) }));
      })
  ...
  const ladeVereinsdaten = async (clubId, profileId, roster) => {
    ...
    const fehlgeschlagen = [
      [tipps.error, t("tipp.tipps")], [ergebnisse.error, t("sys.spielergebnisse")],
  ...
  useEffect(() => {
    ...
    const finishNewClubRegistration = async () => {
      ...
      const { data: registration, error } = await supabase.rpc("register_new_club", {
        ...
        member_name: account.user_metadata?.full_name || account.email?.split("@")[0] || t("rol.mitgliedLabel"),
  ...
  /* Abmelden heisst: nichts vom alten Konto bleibt stehen.
     ...
     gefuellten Mitgliedschaftsliste auf t("verein.deine") des GERADE
     abgemeldeten Kontos fuehrte - und im Anmeldeformular bot die Karte
     t("login.bleiben") Name und E-Mail des Vorgaengers an. Auf einem
  ...
  /* "Alle ansehen" unter den Vereins-News.
     ...
     stattdessen den erstbesten Mannschaftskanal, und wer auf t("allg.alleAnsehen")
  ...
  const themeVars = {
    ...
    /* Was auf der Vereinsfarbe LESBAR ist. Bisher stand ueberall fest Weiss -
       bei einer hellen Primaerfarbe (Gelb, Hellblau) verschwand die Schrift
       darin, und die App half sich mit einem Warnhinweis an den Vereinsadmin.
       Eine Farbe zu waehlen und dann gewarnt zu werden, dass sie nicht
       funktioniert, ist keine Loesung - jetzt passt sich die Schrift an. */
    "--club-primary-kontrast": hexIsLight(clubPrimary) ? "#1B1418" : "#FFFFFF",
    ...
    /* Dasselbe fuer die Zweitfarbe. Sie ist bei mehreren Vorgaben Weiss
       ("Blau & Weiss") und bei einer Schwarz ("Orange & Schwarz") - beide
       Extreme kommen also wirklich vor. */
    "--club-secondary-kontrast": hexIsLight(clubSecondary) ? "#1B1418" : "#FFFFFF",
    /* Gegenstueck zu --club-secondary-light: Die Zweitfarbe als SCHRIFT auf
       heller Flaeche. Bei vier der sieben Vorgaben ist sie Weiss - dort waere
       sie auf einer weissen Karte unsichtbar. Abgedunkelt wird nur, wenn noetig;
       eine ohnehin dunkle Zweitfarbe bleibt, wie sie ist. */
    "--club-secondary-readable": hexIsLight(clubSecondary) ? mischeHex(clubSecondary, 0.45, false) : clubSecondary,
  };
                  ...
                  <button onClick={() => { setSubView("postfach"); postfachLaden(); }} aria-label={`Benachrichtigungen${ungelesen ? `, ${ungelesen} ungelesen` : ""}`}
                    ...
                    {ungelesen > 0 && (
                      <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 rounded-full flex items-center justify-center text-[9px] font-bold"
                        style={{ background: C.red, color: C.aufPrimaer }}>{ungelesen > 99 ? "99+" : ungelesen}</span>
                    )}
                  </button>
                  <button onClick={returnToClubOverview} className="ml-2 px-3 py-1.5 rounded-full text-[10px] font-bold whitespace-nowrap flex-shrink-0" style={{background:C.glass,border:`1px solid ${C.edge}`,color:C.textDim}}>{t("verein.wechseln")}</button>
                </div>
              </div>
            )}
