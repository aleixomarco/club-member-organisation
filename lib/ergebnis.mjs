...
/* Pruefung                                                             */
/* ------------------------------------------------------------------ */
/* Tore: ganze Zahl von 0 bis 99, als Zahl oder als Text aus dem Feld.
   Dieselbe Grenze wie ergebnis_pruefgrund in der Datenbank. */
export function toreGueltig(v) {
  if (typeof v === "number") return Number.isInteger(v) && v >= 0 && v <= 99;
  if (typeof v === "string") return /^\d{1,2}$/.test(v.trim());
  return false;
}
